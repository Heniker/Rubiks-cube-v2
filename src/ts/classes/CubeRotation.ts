...
class CubeRotation extends Toggle {
	...
	rotate(cube: THREE.Mesh, fixedAxle_: string, factor_: 1 | -1) {
		...
		// #Typecasting>
		this.childCubes = (this.cubeGroup.children as THREE.Mesh[]).filter((e) =>
			e.position[this.fixedAxle] === cube.position[this.fixedAxle])
		this.pivotPoint = new THREE.Mesh(new THREE.Geometry(),
	...
	private animationTask() {
		...
		if (this.animationState >= 90) {
			...
			this.pivotPoint['rotate' + this.fixedAxle.toUpperCase()]
